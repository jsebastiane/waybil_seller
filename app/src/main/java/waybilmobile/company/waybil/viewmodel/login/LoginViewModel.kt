package waybilmobile.company.waybil.viewmodel.login
...
class LoginViewModel : ViewModel(){
    ...
    fun getUser(){
        ...
        currentUid?.let {
            mFirebaseDatabase.collection("users").document(it).get().addOnSuccessListener {documentSnapshot ->
                ...
                Log.d("CHECK USER EXISTS:", "Task failed")
                _getUserError.value = false
            }
    ...
    private fun updateFirebaseData(){
        ...
        mFirebaseDatabase.collection("users").document(currentUser!!.id).update(updates)
